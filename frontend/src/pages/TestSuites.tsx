    ...
    <div>
      <div className={styles.pageHeader}>
        <div className={styles.breadcrumb}>
          <span onClick={() => navigate('/')} style={{ cursor: 'pointer', color: 'var(--primary-600)' }}>Home</span>
          <span>›</span>
          <span>Test Suites</span>
        </div>
        <h1 className={styles.pageTitle}>Test Suites</h1>
        <p className={styles.pageSubtitle}>Manage and execute your test collections</p>
      </div>
      ...
      {testSuites.length === 0 ? (
        <div className={styles.emptyState}>
          ...
          <div className={styles.emptyTitle}>No Test Suites Yet</div>
          <div className={styles.emptyDescription}>Create your first test suite to start automating your tests</div>
        ...
        <div className={styles.testSuitesGrid}>
          {testSuites.map((suite) => (
            <div key={suite.id} className={styles.suiteCard} onClick={() => navigate(`/suites/${suite.id}`)} style={{ cursor: 'pointer' }}>
              <div className={styles.suiteHeader}>
                <div className={styles.suiteIcon}>{suite.icon || '📦'}</div>
                <div className={styles.suiteInfo}>
                  <div className={styles.suiteTitle}>{suite.name}</div>
                  <div className={styles.suiteDescription}>{suite.description || 'No description'}</div>
                </div>
              </div>
              <div className={styles.suiteStats}>
                <div className={styles.suiteStat}>
                  <div className={styles.suiteStatIcon} style={{ background: 'var(--success-50)', color: 'var(--success-600)' }}>✓</div>
                  <span>{suite.passed || 0}</span>
                </div>
                <div className={styles.suiteStat}>
                  <div className={styles.suiteStatIcon} style={{ background: 'var(--error-50)', color: 'var(--error-600)' }}>✗</div>
                  <span>{suite.failed || 0}</span>
                </div>
                <div className={styles.suiteStat}>
                  <div className={styles.suiteStatIcon} style={{ background: 'var(--gray-100)', color: 'var(--gray-600)' }}>⊘</div>
                  <span>{suite.skipped || 0}</span>
                </div>
              </div>
              <div className={`${styles.suiteBadge} ${styles[suite.status]}`}>
                <span>●</span>
                <span>{suite.status} • Last run {suite.lastRun || 'never'}</span>
              </div>
              <div className={styles.suiteActions}>
                <Button
                  variant="primary"
                  onClick={(e) => { e.stopPropagation(); handleRunTests(suite.id); }}
                  disabled={isRunning}
                >
                  <span>▶</span>
                  Run
